...
# Add OpenVoice to path - try multiple locations
...
openvoice_path = None
for path in possible_openvoice_paths:
    if path.exists() and (path / "openvoice").exists():
        openvoice_path = str(path)
        sys.path.append(openvoice_path)
        break

if not openvoice_path:
    print("⚠️  OpenVoice models not found in expected locations:")
    for path in possible_openvoice_paths:
        print(f"   - {path}")
    print("Audio generation may not work properly.")
...
# OpenVoice imports - handle missing models gracefully
try:
    ...
    print("Audio generation will be disabled.")
...
def initialize_openvoice():
    ...
    if not OPENVOICE_AVAILABLE:
        logger.warning("OpenVoice not available - audio generation disabled")
        return False
    
    # Check if model files exist
    required_files = [
        f'{EN_CKPT_BASE}/config.json',
        f'{EN_CKPT_BASE}/checkpoint.pth',
        f'{EN_CKPT_BASE}/en_default_se.pth'
    ]
    
    missing_files = [f for f in required_files if not Path(f).exists()]
    if missing_files:
        logger.error("Missing OpenVoice model files:")
        for f in missing_files:
            logger.error(f"  - {f}")
        logger.error("Please download models or run the installer")
        return False
    ...
    try:
        ...
        # Only load Chinese models if they exist
        if Path(f'{ZH_CKPT_BASE}/config.json').exists():
            zh_base_speaker_tts = BaseSpeakerTTS(f'{ZH_CKPT_BASE}/config.json', device=device)
            zh_base_speaker_tts.load_ckpt(f'{ZH_CKPT_BASE}/checkpoint.pth')
        
        # Only load converter if it exists
        if Path(f'{CKPT_CONVERTER}/config.json').exists():
...
def generate_audio_overview(text, output_path):
    ...
    try:
        ...
        if not OPENVOICE_AVAILABLE:
            raise Exception("OpenVoice not available - please install models")
        
        if not en_base_speaker_tts:
            raise Exception("OpenVoice TTS model not initialized - please check model files")
